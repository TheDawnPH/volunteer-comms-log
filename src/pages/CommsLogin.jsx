 ...
 *  1. Scan the QR code from a comms headset (manual entry here — swap in
 *     a camera-based scanner such as `@yudiel/react-qr-scanner` later).
...
export default function CommsLogin() {
  ...
  async function handleScan(e) {
    ...
    try {
      const { data: equipment, error: eqErr } = await supabase
        .from('comms_equipment').select('*').eq('qr_value', qr.trim()).single()
      if (eqErr || !equipment) throw new Error('Unrecognized QR code for a comms headset.')
  ...
  return (
    <div>
      ...
      <p style={{ marginBottom: 20 }}>Scan (or type) the QR value printed on a headset to time in or out.</p>
      ...
      <div className="card card-pad" style={{ maxWidth: 460, marginBottom: 24 }}>
        ...
        <form onSubmit={handleScan}>
          <div className="field">
            <label>Headset QR value</label>
            <input value={qr} onChange={e => setQr(e.target.value)} placeholder="e.g. HEADSET-04" required autoFocus />
            <p className="hint-text">On a phone this field can be filled by a camera QR scanner component.</p>
          </div>
          <button className="btn btn-primary" style={{ width: '100%' }} disabled={busy}>{busy ? 'Checking…' : 'Scan'}</button>
        </form>
      </div>
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: 'time_in', label: 'Time IN', render: r => new Date(r.time_in).toLocaleString() },
          { key: 'time_out', label: 'Time OUT', render: r => r.time_out ? new Date(r.time_out).toLocaleString() : '—' },
